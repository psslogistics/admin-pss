...
import { assignedClients, employee, employeeActivity, employeeNotifications, employeeTasks, hasPermission, type DemoEmployee } from "@/lib/employee-data";

export type EmployeeSearchResult = { id: string; type: string; title: string; detail: string; href: string; keywords: string };

function getEmployeeSearchIndex(currentEmployee = employee): EmployeeSearchResult[] {
  const pages = allNavigationItems.filter((item) => hasPermission(currentEmployee, item.permission)).map((item) => ({ id: `page-${item.href}`, type: "Page", title: item.title, detail: "Workspace page", href: item.href, keywords: `${item.title} ${item.href}` }));
  ...
  const clients = assignedClients.filter((client) => currentEmployee.assignedClientIds.includes(client.id)).map((client) => ({ id: client.id, type: "Client", title: client.name, detail: `${client.contact} · ${client.location} · ${client.status}`, href: `/dashboard/myClients/${encodeURIComponent(client.id)}`, keywords: `${client.name} ${client.contact} ${client.location} ${client.status} ${client.lastActivity}` }));
