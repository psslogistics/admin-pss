...
export default function ShipmentBookingForm({ clientName, onCancel, onSubmit }: Props) {
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!origin.trim() || !destination.trim() || !description.trim() || !isPositive(weight) || !Number.isInteger(count) || count < 1 || !isPositive(value)) {
      setError("Complete the origin, destination, shipment description, weight, box count, and shipment value.");
      return;
    }
    if (!invoices.length) {
    ...
    setReview({ clientName, origin: origin.trim(), destination: destination.trim(), description: description.trim(), weight, pieces: String(count), value, invoices, dimensionMode, dimensions: requiredDimensions });
  };
