...
type PartyAddress = { address: string; city: string; state: string; pincode: string; phone: string };

export type ShipmentBooking = {
  clientName: string;
  consignor: string;
  consignorAddress: PartyAddress;
  consignee: string;
  consigneeAddress: PartyAddress;
  origin: string;
  destination: string;
  description: string;
  weight: string;
...
type Props = { clientName: string; onCancel: () => void; onSubmit: (booking: ShipmentBooking) => void | Promise<void> };
...
export default function ShipmentBookingForm({ clientName, onCancel, onSubmit }: Props) {
  ...
  const [consignorAddress, setConsignorAddress] = useState<PartyAddress>({ address: "", city: "", state: "", pincode: "", phone: "" });
  const [consigneeAddress, setConsigneeAddress] = useState<PartyAddress>({ address: "", city: "", state: "", pincode: "", phone: "" });
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const addressComplete = (party: PartyAddress) => Boolean(party.address.trim() && party.city.trim() && party.state.trim() && /^\d{6}$/.test(party.pincode.trim()) && /^\d{10}$/.test(party.phone.replace(/\D/g, "").slice(-10)));
    if (!consignor.trim() || !consignee.trim() || !addressComplete(consignorAddress) || !addressComplete(consigneeAddress) || !origin.trim() || !destination.trim() || !description.trim() || !isPositive(weight) || !Number.isInteger(count) || count < 1 || !isPositive(value)) {
      setError("Complete consignor and consignee names, full addresses, valid six-digit pincodes, ten-digit phone numbers, origin, destination, and shipment details.");
      return;
    }
    if (!invoices.length) {
    ...
    setReview({ clientName, consignor: consignor.trim(), consignorAddress, consignee: consignee.trim(), consigneeAddress, origin: origin.trim(), destination: destination.trim(), description: description.trim(), weight, pieces: String(count), value, invoices, dimensionMode, dimensions: requiredDimensions });
  };
  ...
  const addressFields = (label: string, value: PartyAddress, setValue: (next: PartyAddress) => void) => <section className="rounded-xl border border-border/70 p-4"><h3 className="text-sm font-semibold">{label} address</h3><div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-semibold">Complete address</span><textarea required value={value.address} onChange={(event) => setValue({ ...value, address: event.target.value })} placeholder="Building, street, locality" className={`${field} h-20 py-2`} /></label><label><span className="mb-1.5 block text-xs font-semibold">City</span><input required value={value.city} onChange={(event) => setValue({ ...value, city: event.target.value })} className={field} /></label><label><span className="mb-1.5 block text-xs font-semibold">State</span><input required value={value.state} onChange={(event) => setValue({ ...value, state: event.target.value })} className={field} /></label><label><span className="mb-1.5 block text-xs font-semibold">Pincode</span><input required inputMode="numeric" pattern="[0-9]{6}" maxLength={6} value={value.pincode} onChange={(event) => setValue({ ...value, pincode: event.target.value.replace(/\D/g, "").slice(0, 6) })} placeholder="110001" className={field} /></label><label><span className="mb-1.5 block text-xs font-semibold">Phone</span><input required inputMode="tel" value={value.phone} onChange={(event) => setValue({ ...value, phone: event.target.value })} placeholder="10-digit phone" className={field} /></label></div></section>;

  return <div className="fixed inset-0 z-50 grid place-items-center bg-background/70 p-4 backdrop-blur-sm" role="presentation" onClick={onCancel}><section role="dialog" aria-modal="true" aria-labelledby="shipment-booking-title" className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-2xl" onClick={(event) => event.stopPropagation()}><div className="flex shrink-0 items-start justify-between gap-4 border-b border-border/70 p-5"><div><p className="text-[10px] font-semibold uppercase tracking-wider text-primary">Frontend booking workflow</p><h2 id="shipment-booking-title" className="mt-1 text-lg font-semibold">Book shipment</h2><p className="mt-1 text-xs text-muted-foreground">{clientName} · invoices and box dimensions are kept with this shipment.</p></div><button type="button" aria-label="Close shipment booking" onClick={onCancel} className="rounded-lg p-1.5 text-muted-foreground hover:bg-accent hover:text-foreground"><X className="size-4" /></button></div><form onSubmit={submit} className="min-h-0 overflow-y-auto p-5"><div className="grid gap-4 sm:grid-cols-2"><label><span className="mb-1.5 block text-xs font-semibold">Consignor name</span><input required value={consignor} onChange={(event) => setConsignor(event.target.value)} placeholder="Sender name" className={field} /></label><label><span className="mb-1.5 block text-xs font-semibold">Consignee name</span><input required value={consignee} onChange={(event) => setConsignee(event.target.value)} placeholder="Receiver name" className={field} /></label></div><div className="mt-4 grid gap-4 lg:grid-cols-2">{addressFields("Consignor", consignorAddress, setConsignorAddress)}{addressFields("Consignee", consigneeAddress, setConsigneeAddress)}</div><div className="mt-4 grid gap-4 sm:grid-cols-2"><label><span className="mb-1.5 block text-xs font-semibold">Origin</span><input required value={origin} onChange={(event) => setOrigin(event.target.value)} placeholder="Pickup city" className={field} /></label><label><span className="mb-1.5 block text-xs font-semibold">Destination</span><input required value={destination} onChange={(event) => setDestination(event.target.value)} placeholder="Delivery city" className={field} /></label><label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-semibold">What are you shipping?</span><input required value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Shipment description" className={field} /></label><label><span className="mb-1.5 block text-xs font-semibold">Total weight</span><div className="relative"><input required type="number" min="0.1" step="0.1" value={weight} onChange={(event) => setWeight(event.target.value)} placeholder="Weight" className={`${field} pr-12`} /><span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground">kg</span></div></label><label><span className="mb-1.5 block text-xs font-semibold">Number of boxes</span><input required type="number" min="1" step="1" value={pieces} onChange={(event) => setPieces(event.target.value)} className={field} /></label><label><span className="mb-1.5 block text-xs font-semibold">Shipment value</span><div className="relative"><span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">₹</span><input required type="number" min="0.1" step="0.01" value={value} onChange={(event) => setValue(event.target.value)} placeholder="Declared value" className={`${field} pl-7`} /></div></label></div>
